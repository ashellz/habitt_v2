...
iOS (App Store): https://apps.apple.com/ba/app/habitt-private-habit-tracker/id6745617462
...
**Habit tracking (duh)**
- Track habits by completion count (amount) or time spent (duration), or just tick them as done
- Flexible schedules: daily, weekly, monthly, specific days of the week/month, or custom intervals (every X days)
...
- Start a timer for your time-spent habit so you don't have to guess how much time you spent and so it autocompletes
- Pause a habit to temporarily exclud it from tracking, scheduling and streaks without deleting it (useful for injury, sickness, travel, etc.)
- Delete a habit once you don't want it anymore of course
...
- Recommendations to increase or lower targets as you build consistency or start flopping
...
**Reminders**
- Per-habit notifications with configurable times, weekdays and personalized texts
- Morning, Mid-day, Wrap up notifications, schedulable per day of the week
- Select from 10 notifications sounds

**Appearance**
- Dark and light mode
- Design done by professional designers, no AI-slop


**Backup**
- Optional cloud backup via Google Drive or iCloud, data stays on your own account, never on our servers
- Create local backups, export/import a backup file directly on your device, no account needed
- Local AES-256-GCM encryption (PBKDF2-HMAC-SHA256, 200 000 iterations), with optional PIN protection on the encryption key
...
***Currently worked on (0):***
(0) Apple Health and Health Connect integrations:
- Sync calories, sleep, workouts, steps and more from your health app to Habitt, this will complete and add progress to your habits automatically
...
This project has been worked on since 2024, well before AI coding became good. The project structure, design, and app logic are all done by me (Shellz), not generated from scratch by AI. This is not a vibecoded app.

Claude is now used throughout this project only as a helper, not being the one doing everything. The focus areas are:

- **Repetitive tasks** — translating to othe rlanguages, widget extracting
- **Quick bug fixes** — investigating and fixing bugs faster than doing it manually because it can spot clear issues faster
